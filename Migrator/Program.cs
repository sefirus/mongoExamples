...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://admin:admin@cluster0.vetppqz.mongodb.net/?retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            if (datePipelineResults.Count > 0)
            {
                ...
                {
                    operations.Add(new ReplaceOneModel<Movie>(
                        filter: Builders<Movie>.Filter.Where(m => m.Id == movie.Id),
                        replacement: movie));
                }
                
                bulkWriteDatesResult = await _moviesCollection.BulkWriteAsync(operations, new BulkWriteOptions()
                {
                    IsOrdered = false
                });
            ...
            if (ratingPipelineResults.Count > 0)
            {
                ...
                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(operations, new BulkWriteOptions()
                {
                    IsOrdered = false
                });
