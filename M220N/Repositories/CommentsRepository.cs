    ...
    public class CommentsRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var filter = Builders<Comment>.Filter
                .Where(c => c.Id == commentId && c.Email == user.Email);

            var update = Builders<Comment>.Update.Set(c => c.Text, comment);

            var updateResult = await _commentsCollection
            ...
            */
            try
            {
                var result = await _commentsCollection
                    .WithReadConcern(new ReadConcern(new Optional<ReadConcernLevel?>(ReadConcernLevel.Majority)))
                    .Aggregate()
                    .Group((Comment comment) => comment.Email,
