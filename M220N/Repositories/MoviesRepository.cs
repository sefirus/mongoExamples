...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            try
            {
                return await _moviesCollection.Aggregate()
                    ...
                    .Lookup(
                        _commentsCollection, m => m.Id, c => c.MovieId, (Movie m) => m.Comments)
                    // Ticket: Get Comments
        ...
        {
            var filter = Builders<Movie>.Filter.Where(movie => movie.Countries.Any(country => countries.Contains(country)));

            var projectionFilter = Builders<Movie>.Projection
                .Include(m => m.Title)
                .Exclude(m => m.Id);

            var moviesProjected = await _moviesCollection
                .Find(filter)
                .Project<MovieByCountryProjection>(projectionFilter)
                .ToListAsync(cancellationToken: cancellationToken);

            return moviesProjected;
        }
            ...
            int page = 0, params string[] genres)
        {
            ...
            var filter = Builders<Movie>.Filter.Where(movie => movie.Genres.Any(genre => genres.Contains(genre)));
