        ...
        */
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                await _usersCollection
                    .WithWriteConcern(new WriteConcern(1))
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filter = Builders<Session>.Filter.Where(session =>
                        session.UserId == storedUser.Id);

                var update = Builders<Session>.Update
                    .Set(session => session.UserId, storedUser.Id)
                    .Set(session => session.Jwt, storedUser.AuthToken);
        ...
        public async Task<UserResponse> LogoutUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            await _sessionsCollection.DeleteOneAsync(
                filter: Builders<Session>.Filter.Where(ses => ses.UserId == user.Id));
        ...
        {
            var user = await GetUserAsync(email, cancellationToken);
            return await _sessionsCollection.Find(Builders<Session>.Filter.Where(ses => ses.UserId == user.Id)).FirstOrDefaultAsync();
        }
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filter = Builders<User>.Filter.Where(user =>
                    user.Email == email);

                var update = Builders<User>.Update
                    .Set(u => u.Preferences, preferences);

                var updateResult = await _usersCollection
                    .UpdateOneAsync(
                        filter, 
                        update, 
                        new UpdateOptions()
